...
from skills.notify import truncate_for_telegram
...
def test_telegram_summary_caps_at_five_jobs():
    many = [dict(JOBS[0], job_id=f"j{i}") for i in range(9)]
    text = build_telegram_summary(many, total_scored=30, role="AI Engineer")
