...
export default function LogPanel({ logs, onClear }: LogPanelProps) {
  ...
  return (
    <div
      ...
      style={{ background: '#030508', borderColor: 'rgba(255,255,255,0.06)' }}
    >
      {/* Header */}
      <div
        ...
        {logs.length === 0 ? (
          ...
          <div className="space-y-0.5">
            {logs.slice(0, 60).map((entry, i) => {
              const tagStyle = Object.entries(TAG_STYLES).find(([k]) => entry.tagClass.includes(k) || entry.tag.toLowerCase().includes(k))?.[1]
              return (
