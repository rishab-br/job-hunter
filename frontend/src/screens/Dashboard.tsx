...
function StatCard({ label, value, icon, accent, sub }: StatCardProps) {
  ...
  return (
    <div
      className="relative overflow-hidden rounded-2xl p-5 flex flex-col justify-between min-h-[130px] transition-all duration-300 hover:scale-[1.01] hover:brightness-110 cursor-default"
      style={{
        background: `linear-gradient(135deg, ${accent}12 0%, rgba(255,255,255,0.03) 60%, transparent 100%)`,
    ...
    >
      <div className="flex items-center justify-between mb-5">
        ...
        <span className="text-[10px] text-slate-600">
          {curIdx >= 0 ? `Step ${curIdx + 1} of ${PIPE_STEPS.length}` : 'Idle'}
        </span>
      </div>

      <div className="flex items-center">
        {PIPE_STEPS.map((step, i) => {
          const isDone   = curIdx > i || !!(summary as unknown as Record<string, unknown>)?.[`has_${step.key.split('_')[0]}`]
            ...
            <div key={step.key} className="flex items-center flex-1 last:flex-none">
              <div className="flex flex-col items-center gap-2.5">
                ...
                >
                  ...
                  {isActive && (
                    <div
                      className="absolute -inset-1 rounded-2xl animate-dot-pulse"
                  ...
                  {isDone && !isActive && (
                    <div
                      className="absolute -top-1 -right-1 w-4 h-4 rounded-full flex items-center justify-center text-[8px] font-bold text-slate-900"
                ...
                {/* Label */}
                <span
                  className="text-[9px] text-center font-semibold whitespace-pre-line leading-tight"
              ...
              {i < PIPE_STEPS.length - 1 && (
                <div className="flex-1 mx-2 mb-6">
                  ...
                  >
                    {(isDone || isActive) && (
                      <div
                        className="pipe-flow absolute inset-0"
...
function ModuleCard({ def, state, metric, onRun, onView, disabled }: ModuleCardProps) {
  const { color, name, metricLabel } = def
  const Icon    = def.icon
  ...
  return (
    <div
      className={`relative overflow-hidden rounded-2xl p-5 flex flex-col gap-3 transition-all duration-300 group ${running ? 'card-running' : ''}`}
      style={{
        background: done
    ...
    >
      ...
      <div
        className="absolute right-2 bottom-2 opacity-[0.04] pointer-events-none transition-opacity group-hover:opacity-[0.07]"
        style={{ color }}
      >
        <Icon size={72} />
      </div>
      ...
      <div className="flex items-start justify-between relative z-10">
        ...
        >
          <span
            className={`w-1.5 h-1.5 rounded-full ${running ? 'animate-dot-pulse' : ''}`}
            style={{ background: statusConfig.dot, boxShadow: running ? `0 0 6px ${statusConfig.dot}` : 'none' }}
          />
      ...
      <div className="relative z-10">
        <div
          className="text-3xl font-black leading-none mb-1 transition-colors"
          style={{
            color: (done || running) ? color : 'rgba(255,255,255,0.25)',
            textShadow: (done || running) ? `0 0 20px ${color}60` : 'none',
          }}
        >
          {String(metric)}
        </div>
...
export default function Dashboard({ summary, moduleStates, onRunModule, onRefresh, activeJobId, onNavigate }: DashboardProps & { onNavigate: (s: string) => void }) {
    ...
    <div className="flex flex-col flex-1 overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5 bg-grid" style={{ backgroundSize: '40px 40px' }}>
        ...
        <div className="grid grid-cols-4 gap-4">
          ...
          <StatCard label="Apps Submitted"     value={s?.apps_submitted  ?? '—'} icon={<Briefcase size={18} />}   accent="#10B981" sub={s?.apps_submitted ? `${s.apps_submitted} total` : undefined} />
        ...
        {/* Pipeline */}
        <Pipeline currentPhase={currentPhase} summary={summary} />
