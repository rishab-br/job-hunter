...
def get_profile(username: str) -> dict:
    user = get_client().get_user(username)
    return {
        "login": user.login,
        "name": user.name,
        "bio": user.bio,
...
def get_readme_content(username: str, repo_name: str) -> str | None:
    try:
        repo = get_client().get_repo(f"{username}/{repo_name}")
...
def get_languages(username: str, repo_name: str) -> dict[str, int]:
    """Returns language → bytes mapping for a repo."""
    try:
        repo = get_client().get_repo(f"{username}/{repo_name}")
